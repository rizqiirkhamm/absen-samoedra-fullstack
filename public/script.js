...
function updateWaktu() {
    ...
    // Hanya update elemen jika mereka ada di halaman
    if (tanggalInput) {
...
// Hanya jalankan update waktu jika elemen form presensi ada
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            fetch(
                ...
                .catch(() => {
                    lokasiInput.value = `Lokasi: ${lat.toFixed(
                        4
                    )}, ${lon.toFixed(4)}`;
                });
        },
...
namaSelect.addEventListener("change", async function () {
    ...
    try {
        ...
        if (data.checkedIn) {
            document
                .getElementById("alreadyCheckedInWarning")
                .classList.remove("hidden");
            document.getElementById("submitBtnText").textContent = "Check Out";
            // Selalu tampilkan field alasan checkout
            document
                .getElementById("earlyLeaveContainer")
                .classList.remove("hidden");
            document
                .getElementById("earlyLeaveReason")
        ...
        showAlert(
            "Error",
            "Terjadi kesalahan saat memeriksa status presensi",
            "error"
        );
    }
});
...
form.addEventListener("submit", async function (e) {
    ...
    if (!canvas.classList.contains("hidden")) {
        ...
        submitBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> ${
            isCheckOut ? "MENYIMPAN CHECK OUT..." : "MENYIMPAN CHECK IN..."
        }`;
